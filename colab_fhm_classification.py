# -*- coding: utf-8 -*-
"""
Google Colab - FHM (Facebook Hateful Memes) Classification Pipeline
Task: Binary Hateful vs Non-Hateful Meme Detection
"""
...
# --- Step 1: Download & Load Original FHM Dataset ---
print("--- Step 1: Loading FHM Dataset ---")
...
df = pd.read_json(fhm_path, lines=True)
print(f"Dataset columns: {df.columns.tolist()}")
print(f"Total rows: {df.shape[0]} | Hateful (1): {df[df['label']==1].shape[0]} | Safe (0): {df[df['label']==0].shape[0]}")
...
train_loader = DataLoader(HMDDataset(X_train, y_train), batch_size=64, shuffle=True)
val_loader = DataLoader(HMDDataset(X_val, y_val), batch_size=64, shuffle=False)
...
model = SafetyClassifier(input_dim=X_train.shape[1]).to(device)
criterion = nn.CrossEntropyLoss()
optimizer = optim.AdamW(model.parameters(), lr=1e-3, weight_decay=1e-2)

# --- Step 5: Model Training & Evaluation ---
print("\n--- Step 5: Training FHM Model ---")
epochs = 15
...
print("\nFHM training complete! Saving model checkpoint...")
torch.save(model.state_dict(), "fhm_safety_classifier.pt")
